...
/*   Updated: 2026/02/02 11:31:57 by gafreire         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_ray_info(int x, t_raycast *ray)
{
	printf("Ray[%3d]: dir(%.2f, %.2f) distance=%.2f side=%d\n", x, ray->dir_x,
		ray->dir_y, ray->perp_wall_dist, ray->side);
}
...
void	test_single_ray(t_game *game, int screen_x)
{
	...
	init_ray_direction(game, &ray, screen_x);
	printf(" *** Camera x: %.3f\n", ray.camera_x);
...
void	test_raycast_sweep(t_game *game)
{
	...
	printf("Player: (%.2f, %.2f) Direction: (%.2f, %.2f)\n", game->player.pos_x,
		game->player.pos_y, game->player.dir_x, game->player.pos_y);
